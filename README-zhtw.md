...
| 系列 | 文章數 | 字數 |
|------|--------|------|
| Computer Architecture | 4 篇 | ~39,700 字 |
...
| Tech Events | 2 篇 | ~45,000 字 |
| Tech Reads | 1 篇 | ~21,000 字 |

**總計**: 100 篇文章，~457,600 字

---

## 📚 文章系列

### 1. Computer Architecture 系列（4 篇）🆕

從架構師的視角理解 CPU 效能設計與異構運算。

**文章 01 - All Roads Lead to IPC**：IPC（Instructions Per Cycle）、Latency vs Occupation、Superscalar、Out-of-Order 執行、分支預測、Cache 效應、ROB 設計

**文章 02 - 異構系統架構**：六大效能法則（Amdahl、Gustafson、USL、Roofline、Little's Law、Queuing Theory）、四大處理器（CPU/GPU/NPU/DPU）、記憶體架構（UMA/CXL/NVLink）、一致性協議、MLIR、Data-Oriented Design

**文章 03 - 應用驅動的架構選型**：TMAM（Top-Down Microarchitecture Analysis）、CPU 譜系對照（ARM Cortex-M/R/A/Neoverse vs RISC-V SiFive/XiangShan/Ventana）、五大設計場景（Ultra-Low Power、Real-Time Embedded、Rich Embedded、Mobile Computing、Cloud & AI Infrastructure）、效能定律應用（Little's Law、Roofline Model、ILP/MLP 分析）、PPA 權衡

**文章 04 - LLM 驅動的 RISC-V 向量程式碼生成與驗證方法論**：IntrinTrans 框架、Multi-Agent FSM（Translator/Compilation/Test/Optimizer）、VLA（Vector Length Agnosticism）、Strip-mining、LMUL 暫存器壓力、Liveness Analysis、Architecture-Aware 護欄、矽後驗證（Trace Encoder/Funnel）、Cache-aware 優化侷限性

**備註**：本系列提供繁體中文和英文兩個版本（獨立撰寫，非翻譯）。

---

### 2. Cache Architecture 系列（6 篇）
...
### 4. Storage Architecture 系列（12 篇）
...
深度解讀經典技術教材與研究論文，連結理論與系統設計實務。

**文章 01 - 資訊理論第一課：從 Shannon 到系統架構**：將資訊理論基礎概念（熵、互資訊、通道容量）與實際系統設計串連。主題包含：Roofline Model 作為熵的上界、Fano 不等式與分支預測、典型性與基準測試方法論、率失真理論與量化、以資訊圖理解記憶體一致性模型。

**備註**：本系列提供繁體中文和英文兩個版本（獨立撰寫，非翻譯）。

---

### 9. Tech Events 系列（2 篇）

聚焦大型技術大會與產品發表，從系統架構與硬體設計觀點整理重點，強調效能定律與基礎設施演進。

**文章 01 - GTC 2026 技術回顧：AI Factory 時代的系統架構演進**：從 NVIDIA Vera CPU、NVFP4 數值格式、NVLink/NVL72 集群，到 AI Factory 基礎設施與容量規劃，從效能定律、離散化推論與大規模系統設計的角度統整 GTC 2026 關鍵內容。

**文章 02 - 打破算力焦慮：Arm AGI CPU 如何重塑 Agentic AI 基礎設施**：從 DGX Spark 悖論到 Meta 異質運算叢集、OpenAI 的 MCTS 推理樹、Cloudflare 邊緣防禦——探討 Arm 136 核 AGI CPU 如何解決 Agentic AI 三大核心挑戰：記憶體頻寬之壁、確定性延遲、機架級經濟學。主題：工作負載的資訊熵分析、CXL 3.0 零拷貝協調、捨棄 SMT 的理論依據、P-E-C Triangle 最佳化、為何 ASIC 無法取代 CPU 在高熵場景中的角色。
